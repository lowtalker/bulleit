...
public abstract class AbstractIOReactor implements IOReactor {
    ...
    private static final Logger logger = Logger.getLogger(AbstractIOReactor.class.getName());
    ...
    public void addChannel(final ChannelEntry channelEntry) {
        ...
        SocketChannel channel = channelEntry.getChannel();
        ...
        if(this.newChannels.isEmpty()){
            System.out.println("channel queue is empty");
        }
    ...
    protected void execute() throws InterruptedIOException, IOReactorException {
        ...
        try {
            ...
            logger.log(Level.INFO, "closeselectorexception {0}", ignore);
        } finally {
            hardShutdown();
        ...
        while ((session = this.closedSessions.poll()) != null) {
            ...
            if (this.sessions.remove(session)) {
                try {
                    ...
                    logger.log(Level.WARNING, "cancelledKeyException {0}", ex);
                }
            }
        }
    }
