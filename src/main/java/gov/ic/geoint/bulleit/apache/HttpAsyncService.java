...
//        final HttpContext context = new BasicHttpContext();  @todo modified
    ...
    public void responseReady(
        ...
        if (state.getRequestState() == MessageState.ACK_EXPECTED) {
            ...
            logger.log(Level.INFO, "HttpAsyncService#responseReady -- HttpResponse.status: {0}", status);
            if (status == 100) {
    ...
    class HttpAsyncExchangeImpl implements HttpAsyncExchange {
        ...
        public void submitResponse(final HttpAsyncResponseProducer responseProducer) {
            ...
            if (this.completed.getAndSet(true)) {
                ...
                final HttpResponse httpResponse = responseProducer.generateResponse();
                final Outgoing outgoing = new Outgoing(
                        this.request, httpResponse, responseProducer, this.context);
